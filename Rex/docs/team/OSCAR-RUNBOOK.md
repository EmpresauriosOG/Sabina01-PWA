...
- [x] `ingredientUtils.ts` Ã¢â‚¬â€ uses `resolveArrayPayload`, `unwrapApiEnvelope`, `getApiErrorMessage`.
- [x] `menuUtils.ts` Ã¢â‚¬â€ uses `resolveArrayPayload`, `unwrapApiEnvelope`, `getApiErrorMessage`.
- [x] `staffUtils.ts` Ã¢â‚¬â€ uses `resolveArrayPayload`, `unwrapApiEnvelope`, `getApiErrorMessage`.
- [x] All 8 KPI hooks Ã¢â‚¬â€ use `resolveArrayPayload` or `resolveObjectPayload` (see B5 for error handling fix).
- [x] Legacy parsing fallbacks not removed Ã¢â‚¬â€ kept for BR-015/BR-016/BR-021 compatibility.
...
#### B1 Subtasks
- [x] Extracted `DishFormFields` component (inside `DishesModal.tsx`) â€” owns the shared form body: basic info, image, type selects, attributes, and ingredient tabs.
- [x] Defined `DishFormData`, `SelectedIngredient`, `IngredientOption` shared types with clear section comments.
- [x] Both render branches (`isNestedInDialog` and standalone Dialog) now use `<DishFormFields {...sharedFieldsProps} />` â€” ~300 lines of copy-paste eliminated (1007 â†’ 711 lines).
- [x] Renamed inner `formData` shadow in `handleSubmit` to `nativeFormData` to make submission path explicit.
- [x] Extracted submit orchestration into `buildDishPayload` (pure payload mapper) and `persistDish` (create/update execution), leaving `handleSubmit` as a thin coordinator.
- [ ] TODO [B2/BR-003]: Canonical mutation path choice (`submitMenuItem` vs `updateMenuItem`) remains blocked by BR-003 and is tracked under B2.
...
#### B3 Subtasks
- [x] `src/shared/state/formSubmissionState.ts` Ã¢â‚¬â€ deleted (3 Zustand booleans: staffFormSubmitted, ingredientFormSubmitted, dishFormSubmitted).
- [x] `StaffForm.tsx` Ã¢â‚¬â€ replaced `setStaffFormSubmitted(true)` with `queryClient.invalidateQueries({ queryKey: ["staff"] })` in both onSubmit and onSubmitModify.
- [x] `Staff.tsx` Ã¢â‚¬â€ removed useEffect watcher and useFormSubmissionStore import.
- [x] `IngredientForm.tsx` Ã¢â‚¬â€ replaced `setIngredientFormSubmitted(true)` (2 occurrences) with `queryClient.invalidateQueries({ queryKey: ["ingredient"] })`.
- [x] `Ingredients.tsx` Ã¢â‚¬â€ removed useEffect watcher, removed unused `refetch`.
- [x] `Dishes.tsx` Ã¢â‚¬â€ removed useEffect watcher, removed unused `refetch`.
- [x] `DeleteDish.tsx` Ã¢â‚¬â€ replaced `setDishFormSubmitted(true)` with `queryClient.invalidateQueries({ queryKey: ["menu"] })`.
- [x] `ModifyDish.tsx` Ã¢â‚¬â€ replaced `setDishFormSubmitted(true)` with `queryClient.invalidateQueries({ queryKey: ["menu"] })`.
- [x] `DishesModal.tsx` Ã¢â‚¬â€ replaced 2 occurrences of `setDishFormSubmitted(true)` with `queryClient.invalidateQueries({ queryKey: ["menu"] })`.
- [x] `UserColumn.tsx` Ã¢â‚¬â€ extracted `StaffActionsCell` named component; replaced imperative `useFormSubmissionStore.getState().setStaffFormSubmitted(true)` with `queryClient.invalidateQueries({ queryKey: ["staff"] })`.
- [x] `IngredientColumn.tsx` Ã¢â‚¬â€ extracted `IngredientActionsCell` named component; replaced with `queryClient.invalidateQueries({ queryKey: ["ingredient"] })`.
- [x] `DishesColumn.tsx` Ã¢â‚¬â€ extracted `DishActiveCell` named component; replaced with `queryClient.invalidateQueries({ queryKey: ["menu"] })`.
...
#### B4 Subtasks
- [x] `StaffForm.onSubmit` â€” normalized from `await fn().then()` + outer try/catch to pure `async/await + try/catch`. Invalidation now consistent with `onSubmitModify`.
- [x] `StaffForm.onSubmitModify` â€” normalized from `.then().catch()` to `async/await + try/catch`.
- [x] `StaffActionsCell.onDelete` (`UserColumn.tsx`) â€” added try/catch with `toast.error(...)` so delete failures are visible instead of silent.
- [x] `staffUtils.ts` â€” already uses contract parsers and throws typed errors; no changes needed.
- [x] `getStaff.ts` â€” query key `["staff", restaurant_id, location_id]` already consistent with invalidation keys in StaffForm and StaffActionsCell.
...
- [x] Added try/catch with `getApiErrorMessage` to all 8 hooks Ã¢â‚¬â€ errors now produce consistent messages instead of raw axios exceptions.
...
- [x] `src/context/AuthContext.tsx` Ã¢â‚¬â€ Supabase auth context, deleted.
- [x] `src/components/forms/Form.tsx` Ã¢â‚¬â€ legacy login form, deleted.
- [x] `src/utils/tableUtils.ts` Ã¢â‚¬â€ no-contract legacy duplicate, deleted (Ian C1).

Deleted this session (zero importers confirmed before deletion):
- [x] `src/components/management/Tables.tsx` Ã¢â‚¬â€ static demo table UI, not routed, no importers.
- [x] `src/components/containers/Restaurants.tsx` Ã¢â‚¬â€ fully commented-out code, no importers.
- [x] `src/shared/constants.ts` Ã¢â‚¬â€ large static fixture menu data, no active importers.
- [x] `src/hooks/tanstack/getOTP.ts` Ã¢â‚¬â€ dead weather API stub with exposed credential (see E2).
...
- [x] Deleted `src/hooks/tanstack/getOTP.ts` Ã¢â‚¬â€ contained exposed RapidAPI key (`X-RapidAPI-Key`) and was a dead weather API stub (not OTP). Zero importers, no active route. Credential should be rotated/revoked by team.

#### E1/E2 Post-Review Fixes (2026-04-06)
- [x] Updated docs/architecture/01-capabilities/legacy-orphans.md to align module status with actual deletions (src/shared/constants.ts moved from keep to archive and deleted modules marked as archived/deleted).
