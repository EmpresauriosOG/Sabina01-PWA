...
type SelectedIngredient = { id: string; quantity: number };
...
interface DishFormFieldsProps {
  ...
  setAttributeInput: React.Dispatch<React.SetStateAction<string>>;
  selectedAttributes: string[];
  handleAddAttribute: (e: React.KeyboardEvent<HTMLInputElement>) => void;
  ...
  ingredientData: { ingredients: IngredientOption[] } | undefined;
  handleIngredientAdd: (ingredientId: string, quantity: number) => void;
}
...
export default function DishModal({
  ...
  const sharedFieldsProps: DishFormFieldsProps = {
    formData,
    setFormData,
    imagePreview,
    handleImageChange,
  ...
  // Nested in ModifyDish dialog — render form only, no Dialog wrapper
  if (isNestedInDialog) {
    return (
      <form onSubmit={handleSubmit} className="max-h-[60vh] overflow-y-auto pr-2">
        <DishFormFields {...sharedFieldsProps} />
  ...
  // Standalone — full Dialog wrapper
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="outline">
          {editItem ? "Modificar platillo" : "Agregar platillo"}
        </Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-[600px] max-h-[85vh] overflow-hidden flex flex-col">
        <DialogHeader>
          <DialogTitle>
            {editItem ? "Modificar platillo" : "Agregar nuevo platillo"}
          </DialogTitle>
          <DialogDescription>
